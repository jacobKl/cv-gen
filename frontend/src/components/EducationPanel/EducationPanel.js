...
function EducationPopup({ education, id }) {
  const iconsSet = {
    ...
    icons: ["fas fa-university", "fas fa-user-graduate", "fas fa-graduation-cap", "fas fa-briefcase", "fas fa-lightbulb", "fas fa-code"],
  };
  ...
  return (
    <EducationPanelWrapper>
      <EducationPanelHeader status={isOpen} onClick={togglePopup}>
        <p>{education.name ? education.name : "Nie podano"}</p>
