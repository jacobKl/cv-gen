...
function SkillPanel({ name, step, dispatch, id }) {
  const iconSet = {
    dispatchTo: "skills",
    icons: ["fas fa-drafting-compass", "fab fa-html5", "fab fa-css3-alt", "fab fa-react", "fab fa-php", "fas fa-database", "fas fa-language", "fas fa-laptop-code", "fas fa-user-check"],
  };
  ...
  return (
    <StyledSkillPicker>
      ...
      <StyledSteps step={step}>
        {steps.map((step) => (
          <StyledStep onClick={() => handleStepClick(step)}></StyledStep>
        ))}
      </StyledSteps>
      <IconPicker set={iconSet} id={id}></IconPicker>
    </StyledSkillPicker>
  );
}
